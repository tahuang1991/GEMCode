...
def plotalleta(pt, pt1, netas, fraction=95, yaxis = "(csc_bending_angle12_xfactor_L1_2)",ME21CSConly=False):
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in allnpar:
		...
		Teffs = makeEffplot_v2(filedirs_v6, "pt", treename, cuts, [ellipse, ellipse], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs ,text,outputdir2+"GEMCSC_ctau0andctau1000_hybrid_eff_20160925_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d_dirinterstation%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		...
		Teffs = makeEffplot_v2(filedirs_v6, "pt", treename, cuts, [ellipse+"&&"+ellipse_d, ellipse+"&&"+ellipse_d], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs ,text,outputdir2+"GEMCSC_ctau0andctau1000_hybrid_eff_20160925_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d_dirinterstation%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
       ...
       makeplots([Teff0, Teff1], legs, text_h,outputdir2+"GEMCSC_ctau0andctau1000_hybrid_eff_20160925_pt%d_ptbg%d_fraction%d_St2eta%dto%d_allnpar_dirinterstation%s"%(pt, pt1, fraction,  int(netas[neta]*10),int(netas[neta+1]*10), extrapic)) 
       """
