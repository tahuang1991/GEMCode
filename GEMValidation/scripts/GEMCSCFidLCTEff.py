...
b1.SetTitle(" "*10+ "Fiducial eff for lowQ stubs in ME41"+" "*15 + "CMS Simulation Preliminary")
b1.SetStats(0)

treename = "GEMCSCAnalyzer/trk_eff_ME41"
den0 = "nlayers_csc_sh_even>=3 && pt>10"
den1 = "((has_alct&2)>0 && (has_clct&2)>0 && quality_alct_even>0 && quality_clct_even>3)"
den2 = "((has_alct&2)>0 && (has_clct&2)>0 && ((quality_alct_even==0 || quality_clct_even==3) && (has_rpc_dg&2)>0))"
#den2 = "((has_alct&2)>0 && (has_clct&2)>0 && ((quality_alct_even==0 || quality_clct_even==3) && (has_gem_dg&2)>0))"
den3 = "(nlayers_csc_sh_even>3 && (has_alct&2)>0 && (has_clct&2)==0 && (quality_alct_even>0) && (has_rpc_dg&2)>0)"
#den3 = "(nlayers_csc_sh_even>3 && (has_alct&2)>0 && (has_clct&2)==0 && (has_gem_copad&2)>0)"
#den = "nlayers_csc_sh_even>=3 && (has_alct&2)>0 && (has_clct&2)>0 && quality_alct_even>0 && quality_clct_even>3"
#den = "nlayers_csc_sh_even>3 && (has_alct&2)>0 && (has_clct&2)>0 && ((quality_alct_even==0 || quality_clct_even==3) && (has_rpc_dg&2)>0)"
#den = "nlayers_csc_sh_even>3 && (has_alct&2)>0 && (has_clct&2)==0 && (quality_alct_even>0) && (has_rpc_dg&2)>0"
...
den = den0+" && "+den2
num = den0+" && "+den2+" && "+"(has_lct&2)>0"
#num = den0+" && (quality_alct_even>-1 || quality_clct_even>2)"
#num = den0+" && (quality_alct_even>0 && quality_clct_even>3)"
#num = den0+" && ((quality_alct_even==0 || quality_clct_even==3) && (has_rpc_dg&1)>0)"
#num = den+" && (has_lct&2)>0"
#num = den+"&& (has_csc_sh&2)>0 "
print "den: ", den
print "num: ", num

#e1 = getEff("GEMCSC_Ana_Test1_CSC4.root",treename,den,num)
#e2 = getEff("GEMCSC_Ana_Test1_Wg3.root",treename,den,num)
#e3 = getEff("100k_Ana_PU140_UpgradeME11ME41.root",treename,den,num)
...
#legend.Draw("same")
tex = ROOT.TLatex(.25,.5,"PU140,Pt>10,even chamber")
tex.SetTextSize(0.05)
tex.SetNDC()
tex.Draw("same")

c1.SaveAs("Fid_100k_ME41_eff_lowQstubs_even_Pt10_PU140.pdf")
c1.SaveAs("Fid_100k_ME41_eff_lowQstubs_even_Pt10_PU140.png")
