...
def plotalleta(pt, pt1, netas, fraction=95, yaxis = "(csc_bending_angle12_xfactor_L1_2)",ME21CSConly=False):
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in allnpar:
		...
		astart = .6
		bstart = .0# not used 
		(maxa, maxb, alpha, x0, y0) = loopEllipse(filedirs_v6, treename, fraction, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, netas[neta], netas[neta+1], dens_L1,text,"Profile_Ellipse_PT_0922_00_norotation/GEMCSC_ctau0andctau1000_profile_20160922_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		...
		Teffs = makeEffplot_v2(filedirs_v6, "pt", treename, cuts, [ellipes, ellipes], netas[neta], netas[neta+1],"true muon p_{T} GeV","Efficiency",legs ,text,"Hybrid_Ellipse_PT_0922_00_norotation/GEMCSC_ctau0andctau1000_eff_20160922_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
       ...
       text_h = "#splitline{Hybrid algorithm}{%.1f<|#eta|<%.1f, p_{T}>%d GeV%s}"%(netas[neta],netas[neta+1], pt, extratxt)
       ...
       makeplots([Teff0, Teff1], legs, text_h,"Hybrid_Ellipse_PT_0922_00_norotation/GEMCSC_ctau0andctau1000_eff_20160922_pt%d_ptbg%d_fraction%d_St2eta%dto%d_allnpar%s"%(pt, pt1, fraction,  int(netas[neta]*10),int(netas[neta+1]*10), extrapic)) 
       """
    ...
    Teffs0_alleta.SetName("hybridpromptmuonpt%dptbg%d"%(pt,pt1))
    Teffs1_alleta.SetName("hybriddisplacedmuonpt%dptbg%d"%(pt,pt1))
    text_alleta = "#splitline{Hybrid algorithm}{%.1f<|#eta|<%.1f, p_{T}>%d GeV%s}"%(netas[0],netas[-1], pt, extratxt)
    makeplots([Teffs0_alleta, Teffs1_alleta], legs, text_alleta,"Hybrid_Ellipse_PT_0922_00_norotation/GEMCSC_ctau0andctau1000_eff_20160922_pt%d_ptbg%d_fraction%d_St2eta%dto%d_allnpar%s"%(pt, pt1, fraction,int(netas[0]*10),int(netas[-1]*10), extrapic)) 
    """
    Teff_out.Reopen("Update")
