...
def getTree(fileName,trk_eff = "trk_eff_ME21"):
...
# total numerator selection cut 
    ...
    L_cut = TCut("halfstrip_odd>0 & pad_odd>0 & pt>10")
#  L_cut = AND(L_cut,ok_clct1,ok_gdg1)
    ...
    topTitle = " " * 11 + "halfstrip VS Pad for odd chamber in ME21" + " " *5 + "CMS Simulation Preliminary"
    LUT = TH2F("LUT", topTitle, stripnBins, smin, smax, padnBins, pmin, pmax)
    LUT.SetXTitle("half-strip in ME21")
    LUT.SetYTitle("Pad")
    
    leg = TLegend(0.25,0.2,.35,0.35, "", "brNDC");
    leg.SetBorderSize(0)
    leg.SetFillStyle(0)
    leg.SetHeader("PU0, with Pt>10")
    leg.Draw("same")
    C = TCanvas("C","",800,600)
...
#    LUT.Draw("same")
    ...
    C.Print("%shalfstrip_pad_ME21_1%s"%(output_dir,ext))
...
# total numerator selection cut 
    ## the extra brackets around the extra_num_cut are necessary !!
    t = getTree("%s%s"%(input_dir, input_file1), trk_eff)
    L_cut = TCut("halfstrip_even>0 & pad_even>0 & pt>10")
    ...
    topTitle = " " * 11 + "halfstrip VS Pad for even chamber in ME21" + " " *5 + "CMS Simulation Preliminary"
    LUT = TH2F("LUT", topTitle, stripnBins, smin, smax, padnBins, pmin, pmax)
    LUT.SetXTitle("half-strip in ME21")
    LUT.SetYTitle("Pad")
...
#    LUT.Draw("same")
    ...
    C.Print("%shalfstrip_pad_ME21_2%s"%(output_dir,ext))
...
#  L_cut = AND(L_cut,ok_clct1,ok_gdg1)
    ...
    topTitle = " " * 11 + "halfstrip VS Mean halfstrip from simhits in ME21" + " " *5 + "CMS Simulation Preliminary"
    LUT = TH2F("LUT", topTitle, stripnBins, smin, smax, padnBins, pmin, pmax)
...
#    LUT.Draw("same")
    ...
    C.Print("%shalfstrip_meanhs_ME21_1%s"%(output_dir,ext))
     

#______________________________________________________________________________________________
def HSToGEMPadLUT_1():

    f_1 = open('LUT/ME21_HsToGemPad_1.txt','r')
    k = 0
    s_bins = "(160,0,160)"
    w_bins = "(112,0,112)"
    p_bins = "(200,0,200)"
    ...
    LUT_low1.SetXTitle("halfstrip in ME21")
...
#    print HS_1
    ...
    leg = TLegend(0.50,0.14,.80,0.4, "", "NDC");
    leg.SetBorderSize(0)
    leg.SetFillStyle(0)
    ...
    C.Print("%shalfstrip_to_GEMPad_ME21_LUT1%s"%(output_dir,ext))


#______________________________________________________________________________________________
def HSToGEMPadLUT_2():

    f_2 = open('LUT/ME21_HsToGemPad_2.txt','r')
    s_bins = "(160,0,160)"
    w_bins = "(112,0,112)"
    p_bins = "(200,0,200)"
    ...
    LUT_low2.SetXTitle("halfstrip in ME21")
    ...
    C.Print("%shalfstrip_to_GEMPad_ME21_LUT2%s"%(output_dir,ext))
